...
def connect():
    ...
    connection = sqlite3.connect("DB\\ufr.db")
    print(f"Connected to the database {'ufr.db'}")

    # Créer un curseur
    cursor = connection.cursor()

    cursor.execute('''
        CREATE TABLE IF NOT EXISTS images_ufr (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            category TEXT NOT NULL,
            site TEXT NOT NULL,
            capteur TEXT NOT NULL,  -- Nouvelle colonne pour le type de capteur
            nom_image TEXT NOT NULL,
            image_json TEXT NOT NULL,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        );
    ''')
    print("La table 'images_ufr' a été créée avec succès.")

    # Créer une table 'json_data'
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS masques_ufr (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            data JSON NOT NULL,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        );
    ''')
    print("La table 'masques_ufr' a été créée avec succès.")
...
def insert(conn, category, site, capteur, nom_image, image_json, created_at=None, table="images"):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(f'''
            INSERT INTO {table} (category, site, capteur, nom_image, image_json, created_at)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (category, site, capteur, nom_image, image_json, created_at))
        conn.commit()
    except sqlite3.Error as e:
        print(e)

def insert_json(conn, data, table="masques_ufr"):
    try:
        created_at = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        cursor = conn.cursor()
        cursor.execute(f'''
...
def select_site_details3(parent):
    top = tk.Toplevel(parent)
    top.title("Saisir les détails du site")

    site_var = StringVar()
    capteur_var = StringVar()

    ttk.Label(top, text="Veuillez entrer le nom du site :").pack(pady=5)
    site_entry = ttk.Entry(top, textvariable=site_var)
    site_entry.pack(pady=5)

    ttk.Label(top, text="Veuillez entrer le nom de capteur :").pack(pady=5)
    capteur_entry = ttk.Entry(top, textvariable=capteur_var)
    capteur_entry.pack(pady=5)

    def confirm():
        site = site_var.get()
        capteur = capteur_var.get()
        if site and capteur:
            top.destroy()
    ...
    return site_var.get(), capteur_var.get()

def upload_images_ufr(frame, category, table="images_ufr"):
    conn = connect()

    site, capteur = select_site_details3(frame.winfo_toplevel())  # Demander à l'utilisateur de saisir les détails
        ...
        if folder_path:
            ...
            for index, folder in enumerate(image_files):
                ...
                if os.path.isfile(input_image_path):
                    ...
                    insert(conn, category, site, capteur, 'input.png', json.dumps(image_json), table=table)
...
def upload_masques_ufr(frame, category):
    conn = connect()
    ...
    if file_path:
        try:
            with open(file_path, 'r', encoding='utf-8') as json_file:
                ...
                table = "masques_ufr"  # Vous pouvez adapter cela selon vos besoins
