    ...
    for i, (c, b) in enumerate(zip(obj_categories, bboxes)):
        if i not in ignore:
            ...
            if labels:
                ...
                # Estimer la taille du texte en fonction de sa longueur et de la hauteur de police
                tw = len(text) * 8  # Approximation de la largeur du texte
                th = label_size  # Utiliser la taille de police pour la hauteur du texte

                tx0 = b[0]
                ty0 = b[1] - th

                # Ajuster la position du texte pour s'adapter à l'intérieur de l'image
